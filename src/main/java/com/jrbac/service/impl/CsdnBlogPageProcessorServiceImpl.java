...
import com.jrbac.entity.CsdnBlog;
...
/**
 * CSDN博客爬虫 可以爬取指定用户的csdn博客所有文章，并保存到数据库中。
 * 
 * @author Administrator
 */
@Service
public class CsdnBlogPageProcessorServiceImpl implements PageProcessor {
    private CsdnBlogDao csdnBlogDao;

    public CsdnBlogPageProcessorServiceImpl() {
        super();
    }

    public CsdnBlogPageProcessorServiceImpl(CsdnBlogDao csdnBlogDao) {
        this.csdnBlogDao = csdnBlogDao;
    }
    ...
    public void process(Page page) {
        ...
        if (!page.getUrl().regex("http://blog\\.csdn\\.net/" + username + "/article/details/\\d+").match()) {
            ...
            page.addTargetRequests(page.getHtml().xpath("//div[@id='article_list']").links()// 限定文章列表获取区域
                            .regex("/" + username + "/article/details/\\d+").replace("/" + username + "/", "http://blog.csdn.net/" + username + "/")// 巧用替换给把相对url转换成绝对url
                            .all());
            // 添加其他列表页
            page.addTargetRequests(page.getHtml().xpath("//div[@id='papelist']").links()// 限定其他列表页获取区域
                            .regex("/" + username + "/article/list/\\d+").replace("/" + username + "/", "http://blog.csdn.net/" + username + "/")// 巧用替换给把相对url转换成绝对url
                            .all());
        }
        else {
            // 文章页
            ...
            // 设置阅读人数
            csdnBlog.setView(Integer.parseInt(page.getHtml().xpath("//div[@class='article_r']/span[@class='link_view']").regex("(\\d+)人阅读").get()));
            // 设置评论人数
            ...
            // 把对象存入数据库
            csdnBlogDao.insertSelective(csdnBlog);
            // 把对象输出控制台
    ...
    public static String crawlerRun(CsdnBlogDao csdnBlogDao) {
        long startTime, endTime;
        System.out.println("【爬虫开始】请耐心等待一大波数据到你碗里来...");
        startTime = System.currentTimeMillis();
        // 从用户博客首页开始抓，开启5个线程，启动爬虫
        Spider.create(new CsdnBlogPageProcessorServiceImpl(csdnBlogDao)).addUrl("http://blog.csdn.net/" + username).thread(5).run();
        endTime = System.currentTimeMillis();
        String val = "【爬虫结束】共抓取" + size + "篇文章，耗时约" + ((endTime - startTime) / 1000) + "秒，已保存到数据库，请查收！";
